# gguf-diffusion.cpp Server APIs
...
Native async API for `gguf-diffusion.cpp`.
...
- extend `OpenAI API` requests with native `gguf-diffusion.cpp` controls
- extend `sdapi` requests with native `gguf-diffusion.cpp` controls
...
This allows `sdapi` clients to use native `gguf-diffusion.cpp` controls without changing the outer request format.
...
This is the native `gguf-diffusion.cpp` API.
